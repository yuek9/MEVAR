...
## here try to compute the true Y covariance matrix for decorrelation (based on Bi); 
## but this is a bit strange since previously we treat correlation coming from obs sharing Bi wihtin the cluster
## anyway lets try it
...
if(test_on_box){
  inputs = c(1, 30, 80, 150, 1, 3) #(iter, p, n, Time, infer_row, setting)
...
#####################
## function to compute the theoretical covariance matrix: (refer to https://www.asc.ohio-state.edu/de-jong.8/note6.pdf)

compute_auto_cov = function(A, K, p, Sigma_eps, output_lag){ # input the companion format of the VARd, and provide lag K and dimension p, and noise covariance matrix Sigma_eps
  ## first compute the first K covariances Gamma0, Gamma1,... Gamma(K-1)
  F_mat = A ## A is the companion form
  Q_mat = diag(0, K*p)
  Q_mat[1:p, 1:p] <- Sigma_eps
  
  vec_Sigma_first = solve(diag(1, K^2*p^2) - kronecker(F_mat, F_mat), as.vector(Q_mat))
  
  Sigma_first = matrix(vec_Sigma_first, K*p, K*p)
  
  auto_cov_list = list()
  for (i in 1:K){
    auto_cov_list[[as.character(i-1)]] = Sigma_first[1:p, (i*p-p+1):(i*p)]
  }
  
  ## then compute all auto cov
  
  ## first get the list of transition matrix: from A1 to Ak
  A_list = lapply(1:K, function(i){F_mat[1:p, (i*p-p+1):(i*p)]})
  
  for (i in (K+1):output_lag){
    auto_cov_list[[as.character(i-1)]] = Reduce('+', lapply(1:K, function(w){
      A_list[[w]] %*% auto_cov_list[[as.character(i-1-w)]]
    }))
  }
  
  return(auto_cov_list)
...
for (i in 1:n){
  ...
  while(max(abs(eigen(B_i + companion_Phi)$value))>=1 & counter < 10){
    counter = counter +1
    B_i = matrix(
      rnorm(n = (dim(B_sd_companion)[1])^2, 
            mean=0, 
            sd = as.vector(B_sd_companion)), 
      p*K, p*K) * B_pos_companion
  }
  ...
  ## the theoretical cov (condition on Bi)
  tmp = compute_auto_cov(A = companion_Phi + B_i,
                        K=K,
                        p=p,
                        Sigma_eps = Sig_e,
                        output_lag = Time
...
## check if the population is centered at the right value

range(Reduce('+', record_Bi)/n)

(Reduce('+', record_Bi)/n)[1,1]

##################
## prepare the covariance matrix for input_y for each subject
input_cov = list()
for(i in 1:n){
  tmp_mat = toeplitz(sapply(true_auto_cov[[i]], function(x)x[infer_row, infer_row])[1:(Time-K)])
  cov_y = diag(0, Time-K)
  cov_y[upper.tri(cov_y)] <- tmp_mat[upper.tri(tmp_mat)]
  cov_y = cov_y + t(cov_y) + diag(diag(tmp_mat))
  
  ## then reorder to be Y2 to YT
  cov_y = cov_y[(Time-K):1,][, (Time-K):1]
...
names(input_cov) <- as.character(unique(grp))
...
res.opta = Fixed_effects_est_inf(X=input_X, y=input_y, Z=input_X, grp=grp, a=a.opt, lm=F, inf.coord=inf.coord, 
                                 ...
                                 time_series_cv = time_series_cv, extra_para = list(K=K), true_cov = input_cov )
t2 = toc(quiet = T)
